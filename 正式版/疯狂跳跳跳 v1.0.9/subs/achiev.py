...
class Achiev:
    ...
    def check(self,achiev):
        ...
        if achiev not in self.achievs_list:
            add_error("[error:achiev,check]achiev不存在")
            return False
        ...
        if self.game.time>self.game.user.best_time and self.game.user.best_time>0 and self.game.heart<=0:
            ...
            return True
        elif achiev=="jump_out" and self.game.player.sprites()[0].rect.bottom<=0:
            return True
        elif achiev=="didn't_hurt_in_60s" and self.game.time>=60 and self.game.game_stats.query("hurt","cnt")==0:
            return True
        elif achiev=="hurt_50_times" and self.game.game_stats.query("hurt","cnt")>=50:
            return True
        elif achiev=="hurt_40_in_0.1s" and self.game.time>=0.1 and self.game.game_stats.query("hurt","sum")-self.game.game_stats_list[min(len(self.game.game_stats_list)-1,49)]["hurt"]["sum"]>=40:
            return True
        elif achiev=="4_floor" and len(self.game.floor.sprites())-1>=4:
            return True
        return False
