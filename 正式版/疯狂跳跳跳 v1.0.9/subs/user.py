...
class User:
    ...
    def get_user_data(self):
        ...
        if not "settings" in self.users_data[self.user_name]:
            self.users_data[self.user_name]["settings"]=def_settings
        elif self.users_data[self.user_name]["settings"].keys()!=def_settings.keys():
            for key in def_settings.keys():
                if key not in self.users_data[self.user_name]["settings"]:
                    self.users_data[self.user_name]["settings"][key]=def_settings[key]
    ...
    def update_score(self):
        if self.user_name in self.users_data and self.game.time>=self.best_time and self.game.heart<=0 and self.game.debug==False:
