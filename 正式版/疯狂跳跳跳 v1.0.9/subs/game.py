...
from subs.ganeStats import GameStats
import pygame as pg
import random as rd
...
class Game:
    ...
    def leaderboard(self):
        ldb=pg.sprite.Group()
        ldb=Info.add_info(ldb,f"排行榜",[w/2,40],time=-1,align="center",back_color=None,font=cn_big)
        users={}
        ...
        for u in users:
            ...
            ldb=Info.append_info(ldb,f"{index}. {name}:{st}",[50,70],color=color,back_color=None,time=-1,spacing=5,mode="back")
        ldb=Info.append_info(ldb,f"按Enter退出排行榜",[50,70],time=-1,back_color=None,spacing=5,mode="back")
        window.blit(gray_img,(0,0))
        ldb.draw(window)
        pg.display.update()
        running=True
        ...
        for p,e_l in co.items():#血包碰撞检测
            for e in e_l:
                if e.b==40:
                    ...
                    self.game_stats.add("health/super_health",1,25)
                else:
                    self.heart+=int(e.h)
    ...
    def update(self,events):
        ...
        if len(error_msg)>0:
            for msg in error_msg:
                self.achiev_info=Info.append_info(self.achiev_info,msg,[0,cn_def_sz*2],font=cn_sm,color="red",time=3,max_len=10)
            error_msg.clear()
        if self.developer_mode:
            for ev in events:
                if ev.type==pg.MOUSEBUTTONDOWN:
                    self.achiev_info=Info.append_info(self.achiev_info,f"[开发者模式]位置：{ev.pos}",[0,cn_def_sz*2],font=cn_sm,color="yellow",time=3,max_len=10)
        ...
        if self.developer_mode:
            ...
            self.is_debug=False
        self.game_stats_list.append(self.game_stats.copy_stats())
        if len(self.game_stats_list)>60:
            self.game_stats_list.pop(0)
    def run(self):
        self.game_start()
        while True:
            self.reset()
