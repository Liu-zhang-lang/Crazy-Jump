...
class Debug:
    ...
    def exec_node(self,node,path):
        if node["tag"]=="down":
            ...
            elif cmd=="deve" and node["tip"]=="输入命令":
            ...
            try:
                ...
                add_error("[error:Debug,exec_node]数据类型错误？")
            except NameError:
                add_error("[error:Debug,exec_node]函数不存在")
            except:
                add_error(f"[error:Debug,exec_node]未知错误/(提示："+node["tip"]+")内部错误")
        else:
            add_error("[error:Debug,exec_node]未知的tag")
