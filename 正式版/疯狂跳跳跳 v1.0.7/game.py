...
class Game:
    ...
    def rules(self):
        info=pg.sprite.Group()
        info=Info.append_info(info,"游戏规则&玩法：",[50,60],mode="back")
        ...
        info=Info.append_info(info,"进入账号界面，按Ctrl+M返回主页（不保存进度）",[50,60],spacing=5,mode="back")
        info=Info.append_info(info,"在游戏结束后，按下G查看规则",[50,60],spacing=5,mode="back")
        info=Info.append_info(info,"若在运行过程中在终端发现[error...]的错误，请报告给作者",[50,60],spacing=5,mode="back")
    ...
    def leaderboard(self):
        ...
        for u in users:
            ...
            if index==1:
                ...
                color="white"
            if u["best_minutes"]>0:
                st=f"{u['best_minutes']}分{u['best_seconds']:.2f}秒（{u['best_version']}版本创下）"
            else:
                st=f"{u['best_seconds']:.2f}秒（{u['best_version']}版本创下）"
    ...
    def main_menu(self):
        ...
        info=Info.append_info(info,f"欢迎回来，{self.user.user_name}",[50,50],spacing=5,back_color=None,mode="back")
        info=Info.append_info(info,"按U进入账号界面",[50,50],spacing=5,back_color=None,mode="back")
        info=Info.add_info(info,"作者：Liu_zhang_lang",[50,550],back_color=None)
        button.add(Button((50,120),250,50,"blue",text="开始游戏"))
        button.add(Button((750,550),200,30,"blue",align="topright",text="复制github仓库链接",font=cn_sm,mode=2))
        if self.check_chinese():
            short_info=Info.append_info(short_info,"[警告]当前输入法为中文，可能会使字母按键失效",[320,50],color="orange",time=3,font=cn_sm,spacing=5,back_color=None)
        self.draw()
            ...
            for ev in events:
                ...
                if ev.type==pg.KEYDOWN:
                    ...
                    if ev.key==pg.K_RETURN:
                        return
                for b in button:
                    if b.text=="开始游戏" and b.check_clicked():
                        return
                    if b.text=="复制github仓库链接" and b.check_clicked():
                        pc.copy("https://github.com/Liu-zhang-lang/Crazy-Jump")
                        short_info=Info.append_info(short_info,"链接复制成功",[320,50],color="green",font=cn_sm,spacing=5,back_color=None)
            self.draw()
            window.blit(gray_img,(0,0))
            info.draw(window)
            short_info.draw(window)
