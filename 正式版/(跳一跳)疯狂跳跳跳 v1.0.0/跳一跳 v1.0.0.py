...
class enemy(Sprite):
    def __init__(self,col):
        ...
        self.b=rd.randint(15,35)
        self.image=pg.Surface((self.b,self.b))
        pg.draw.rect(self.image,col,(0,0,self.b,self.b))
        if rd.random()>0.5:
...
def draw():
    ...
    if seconds/60>45 and seconds/60<=47:
        ...
        window.blit(text,text.get_rect(center=(w/2,h/2-200)))
    if jia>jian:
        tt=chinese_font.render("生命值+1",True,"green")
        window.blit(tt,tt.get_rect(topright=(w,25)))
    elif jian>0:
        tt=chinese_font.render("生命值-1",True,"red")
        window.blit(tt,tt.get_rect(topright=(w,25)))
    pg.display.update()
#################################主程序#################################
text1=chinese_font.render("游戏规则&玩法：",True,"white")
text2=chinese_font.render("1：使用左右键或AD键进行左右移动，空格键、W键或上键进行跳跃",True,"white")
text3=chinese_font.render("2：按下ESC暂停，再次按下ESC继续",True,"white")
text4=chinese_font.render("3：每次生成有小概率生成速度极快的敌人",True,"white")
text5=chinese_font.render("4：躲避红色敌人，拾取绿色血包，看看你能坚持多久",True,"white")
text6=chinese_font.render("5：输入法为中文时，可能会导致部分按键失效",True,"white")
text7=chinese_font.render("阅读完毕后，按下回车键开始游戏",True,"white")
draw()
...
while running:
    ...
    if pg.sprite.groupcollide(player,he,False,True):
        heart+=1
        jia=120
    if pg.sprite.groupcollide(player,g,False,True):
        heart-=1
        jian=120
    draw()
    if heart<=0:
