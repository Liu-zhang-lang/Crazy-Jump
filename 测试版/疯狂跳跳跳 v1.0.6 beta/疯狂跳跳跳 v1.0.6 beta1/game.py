...
class Game:
    ...
    def draw(self):
        ...
        text={
            f"时间:{self.seconds:.2f}":[(0,0),chinese_font,"white"],
    ...
    def check_collisions(self):
        ...
        if self.god_mode==False:
            ...
            for p,e_l in co.items():#雨碰撞检测
                for e in e_l:
                    self.heart-=5
                    self.info=Info.add_info(self.info,"生命值-5","red")
    def update(self):
        self.enemy.update()
        self.health.update()
        self.player.update()
        ...
        for i in range(len(level_up_time)):#难度增加
            if level_up_time[i][0]<=self.seconds<=level_up_time[i][1]:
                self.pro=all_pro[i]
