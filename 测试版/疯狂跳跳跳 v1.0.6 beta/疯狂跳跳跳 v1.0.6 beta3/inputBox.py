...
class InputBox(pg.sprite.Sprite):
    def __init__(self,x,y,width,height,outline_color,fill_color,tip,align="topleft",is_selected=False,outline_width=2,font=chinese_font,font_size=chinese_font_size):
        super().__init__()
        ...
        if align=="topright":
            ...
            print("[error:InputBox,__init__]:对齐方式不存在")
        ...
        for ev in events:
            if ev.type==pg.MOUSEBUTTONDOWN:
                if self.rect.collidepoint(ev.pos):
                    ...
                    self.cursor_left_text_width=self.input_text_img.get_width()
                else:
                    self.is_selected=False
                    self.cursor_is_show=False
                    self.cursor_change_time=0.5
