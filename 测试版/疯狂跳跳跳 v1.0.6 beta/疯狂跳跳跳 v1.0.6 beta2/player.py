import pygame as pg
from const import *
class Player(pg.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.pw=30
        self.ph=60
        self.image=pg.Surface((self.pw,self.ph))
        pg.draw.rect(self.image,"blue",(0,0,self.pw,self.ph))
        self.rect=pg.rect.Rect(0,440,self.pw,self.ph)
        self.rect.centerx=window.get_rect().centerx
        self.velocity_y=0
        self.jumping=2#剩余跳跃次数
        self.jumping_wait=0#跳跃冷却时间
    def update(self):
        keys=pg.key.get_pressed()
        if keys[pg.K_LEFT] or keys[pg.K_a]:
            self.rect.x-=5
        if keys[pg.K_RIGHT] or keys[pg.K_d]:
            self.rect.x+=5
        if self.rect.x<0:
            self.rect.x=0
        if self.rect.x+self.pw>w:
            self.rect.x=w-self.pw
        if (keys[pg.K_SPACE] or keys[pg.K_UP] or keys[pg.K_w]) and self.jumping>0 and self.jumping_wait<=0:
            self.jumping-=1
            self.jumping_wait=0.3
            self.velocity_y=-14
        if self.jumping_wait>0:
            self.jumping_wait-=1/60
        self.rect.y+=self.velocity_y
        self.velocity_y+=0.8
    def touch_floor(self,floor):
        if self.rect.bottom-self.velocity_y<=floor.rect.bottom:
            self.rect.bottom=floor.rect.top
            self.jumping=2
            self.velocity_y=0
        elif self.rect.top-self.velocity_y>=floor.rect.top:
            self.rect.top=floor.rect.bottom
            self.velocity_y=0
        elif self.rect.right>=floor.rect.left and self.rect.left<floor.rect.left:
            self.rect.right=floor.rect.left
        else:
            self.rect.left=floor.rect.right