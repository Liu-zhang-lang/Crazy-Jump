...
class Game:
    ...
    def rules(self):
        ...
        info=Info.append_info(info,"进入账号界面，按Ctrl+M返回主页（不保存进度），按S进入设置",[50,60],spacing=5,mode="back")
    ...
    def main_menu(self):
        ...
        if self.auto_login:
            short_info=Info.append_info(short_info,"[账号]自动登录成功",[320,50],color="green",time=3,font=cn_sm,spacing=5,back_color=None)
            self.auto_login=False
        self.draw()
        ...
        for p,e_l in co.items():#血包碰撞检测
            for e in e_l:
                if e.b==40:
                    ...
                    self.heart+=int(e.h)
                    self.info=Info.append_info(self.info,f"生命值+{int(e.h)}",info_st_pos,"topright",color="green",time=2)
