...
class InputBox(pg.sprite.Sprite):
    def __init__(self,x,y,width,height,outline_color,fill_color,tip,align="topleft",text="",is_selected=False,outline_width=2,max_len=20,font=cn_def,multiple_inputs=False,hidden_string=False):
        super().__init__()
        ...
        self.font=font
        self.font_size=self.font.get_height()
        self.tip=tip
        ...
        if text!="":
            if self.hidden_string:
                self.cursor_left_text_width=self.font.render("*"*len(text),True,"white").get_width()
            else:
                self.cursor_left_text_width=self.font.render(text,True,"white").get_width()
