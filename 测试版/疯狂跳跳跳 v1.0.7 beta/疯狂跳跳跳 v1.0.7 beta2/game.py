...
class Game:
    ...
    def rules(self):
        ...
        info=Info.append_info(info,"进入账号界面",[50,60],spacing=5,mode="back")
    ...
    def leaderboard(self):
        ...
        for u in users:
            ...
            if index==1:
                ...
                color="white"
            if u["best_minutes"]>0:
                ldb=Info.append_info(ldb,f"{index}. {name}:{u["best_minutes"]}分{u["best_seconds"]:.2f}秒",[50,50],color=color,time=-1,spacing=5,mode="back")
            else:
                ldb=Info.append_info(ldb,f"{index}. {name}:{u["best_seconds"]:.2f}秒",[50,50],color=color,time=-1,spacing=5,mode="back")
