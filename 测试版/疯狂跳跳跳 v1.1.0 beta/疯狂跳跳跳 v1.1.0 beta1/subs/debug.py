...
class Debug:
                    ...
                    "subs":{
                        ...
                        "bullet":{
                            ...
                            "params":[
                                ...
                                {"type":int,"tip":"x坐标","range":(-1,w),"nor_val":-1},
                                {"type":int,"tip":"y坐标","range":(-1,h),"nor_val":-1},
                                {"type":int,"tip":"角度","range":(-1,360),"nor_val":-1},
                                {"type":int,"tip":"速度","range":(-1,inf),"nor_val":-1},
                                {"type":int,"tip":"伤害","range":(-1,inf),"nor_val":-1}
                            ],
                            "func":self.spawn_bullet
                        }
                    }
                },
                "god":{
                ...
                "set":{
                    ...
                    "subs":{
                        ...
                        "hp":{
                            ...
                            "tip":"设置HP",
                            "params":[
                                {"type":int,"tip":"HP","range":(0,inf),"nor_val":0}
                            ],
                            "func":self.set_hp
                        }
                    }
                ...
                "clear":{
                    ...
                    "subs":{
                        ...
                        "boss":{
                            ...
                            "tip":"清除boss",
                            "params":[],
                            "func":self.clear_boss
                        },
                        "bullet":{
                            "tag":"run",
                            "tip":"清除子弹",
                            "params":[],
                            "func":self.clear_bullet
                        }
                    }
                }
            }
    ...
    def exec_node(self,node,path):
        if node["tag"]=="down":
            ...
            elif self.game.user.encrypt_password(cmd)=="3b0ed08d4ec28826c986521b3acbe748bb59d62f37fec5ddccb98cb1c986b0bb" and node["tip"]=="输入命令":
                self.game.developer_mode=not self.game.developer_mode
            ...
            try:
                ...
                if not self.game.developer_mode:
                    if node["tip"]!="退出debug模式":
                        self.game.game_stats.add("debug",1)
                        self.game.is_debug=True
            except TypeError:
                add_error(f"[error:Debug/exec_node]数据类型错误/函数传参数量不匹配？")
            except NameError:
                add_error(f"[error:Debug/exec_node]函数不存在")
            except:
                add_error(f"[error:Debug/exec_node]未知错误/(提示：{tip})/内部错误")
        else:
            add_error(f"[error:Debug/exec_node]未知的tag({node['tag']})")
    ...
    def spawn_rain(self,n,l):
        for i in range(n):
            self.game.rain.add(Rain(l,draw_num=self.game.user.settings["rain_show"]))
    def spawn_floor(self,n,x,y,l):
        for i in range(n):
            self.game.floor.add(Floor(x,y,l))
    def spawn_boss(self,n):
        for i in range(n):
            self.game.boss.add(Boss(self.game))
    def spawn_bullet(self,n,x,y,a,s,d):
        for i in range(n):
            self.game.bullet.add(Bullet(x,y,a,s,d,draw_num=self.game.user.settings["bullet_show"]))
    def god(self):
    ...
    def clear_boss(self):
        self.game.boss.empty()
    def clear_bullet(self):
        self.game.bullet.empty()
        ...
        for i in range(len(level_up_time)):
            ...
            if self.game.time>=l and self.game.time<n:
                ...
                if self.game.level==8:
                    if self.game.mode=="boss":
                        if self.game.spawn_boss==False:
                            self.game.spawn_boss=True
                            self.game.boss.add(Boss(self.game))
                    else:
                        self.game.pro=0.07
                        self.game.hpro=0.0075
                        self.game.fpro=0.0017
                        self.game.rpro=0.045
                break
    def set_hp(self,h):
        self.game.heart=h
    def start_help(self):
