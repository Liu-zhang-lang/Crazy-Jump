...
class Settings:
    ...
    def game_settings(self):
        ...
        tip4=cn_fonts[17].render("开启后会导致子弹碰撞体积增大",True,"orange")
        tip5=cn_fonts[17].render("开启后需按两次R键才能重开",True,"white")
        ...
        self.button.add(Button((50,y4),150,50,"blue",text="子弹显示攻击力",font=cn_sm))
        self.button.add(Button((50,y5),150,50,"blue",text="重开按键防误触",font=cn_sm))
        for s in def_settings:
            if s not in self.game.user.settings:
                self.game.user.settings[s]=def_settings[s]
        while True:
            ...
            for bt in self.button:
                if bt.check_click():
                    if bt.text=="敌人显示攻击力":
                        ...
                        self.game.user.update_data()
                    elif bt.text=="子弹显示攻击力":
                        self.game.user.settings["bullet_show"]=not self.game.user.settings["bullet_show"]
                        for bullet in self.game.bullet:
                            bullet.draw_pic(self.game.user.settings["bullet_show"])
                        self.game.user.update_data()
            ...
            window.blit(cn_big.render(("开" if self.game.user.settings["bullet_show"] else "关"),True,"white"),(210,y4))
            window.blit(tip4,(50,y4+50))
            window.blit(cn_big.render(("开" if self.game.user.settings["safe_restart"] else "关"),True,"white"),(210,y5))
            window.blit(tip5,(50,y5+50))
            pg.display.update()
            clock.tick(60)