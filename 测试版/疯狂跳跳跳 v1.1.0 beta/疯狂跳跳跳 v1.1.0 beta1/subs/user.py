...
class User:
    def __init__(self,game):
        ...
        with open("D:\\Crazy-Jump\\userdata.json","r",encoding="utf-8") as f:
            ...
            self.migrate_version(format)
            self.users_data=self.data["users"]
            self.version=self.data["version"]
            self.format=self.data["format"]
            self.save_data()
    def migrate_version(self,to_format=format):
        ...
        if to_format-self.format==1:
            if self.format==1:
                ...
                for u in users:
                    best_time=users[u]["best_time"]
                    best_minutes=users[u]["best_minutes"]
                    best_seconds=users[u]["best_seconds"]
                    best_version=users[u]["best_version"]
                    users[u].pop("best_time")
                    users[u].pop("best_minutes")
                    users[u].pop("best_seconds")
                    users[u].pop("best_version")
                    users[u]["best"]={
                        "normal":{
                            "time":best_time,
            ...
            self.migrate_version(to_format-1)
    ...
    def check_user_data(self):
        for user_name in self.users_data:
            if not "best" in self.users_data[user_name]:
                self.users_data[user_name]["best"]={}
            if not "normal" in self.users_data[user_name]["best"]:
                self.users_data[user_name]["best"]["normal"]={}
            if not "time" in self.users_data[user_name]["best"]["normal"]:
                self.users_data[user_name]["best"]["normal"]["time"]=0
            if not "seconds" in self.users_data[user_name]["best"]["normal"]:
                self.users_data[user_name]["best"]["normal"]["seconds"]=0
            if not "minutes" in self.users_data[user_name]["best"]["normal"]:
                self.users_data[user_name]["best"]["normal"]["minutes"]=0
            if not "version" in self.users_data[user_name]["best"]["normal"]:
                self.users_data[user_name]["best"]["normal"]["version"]="Unknown"
            if not "boss" in self.users_data[user_name]["best"]:
                self.users_data[user_name]["best"]["boss"]={}
            if not "time" in self.users_data[user_name]["best"]["boss"]:
                self.users_data[user_name]["best"]["boss"]["time"]=inf
            if not "seconds" in self.users_data[user_name]["best"]["boss"]:
                self.users_data[user_name]["best"]["boss"]["seconds"]=inf
            if not "minutes" in self.users_data[user_name]["best"]["boss"]:
                self.users_data[user_name]["best"]["boss"]["minutes"]=inf
            if not "version" in self.users_data[user_name]["best"]["boss"]:
                self.users_data[user_name]["best"]["boss"]["version"]="Unknown"
            if self.users_data[user_name]["best"]["boss"]["time"]==0:
                self.users_data[user_name]["best"]["boss"]["time"]=inf
                self.users_data[user_name]["best"]["boss"]["seconds"]=inf
                self.users_data[user_name]["best"]["boss"]["minutes"]=inf
                self.users_data[user_name]["best"]["boss"]["version"]="Unknown"
            if not "settings" in self.users_data[user_name]:
                self.users_data[user_name]["settings"]=def_settings
            elif self.users_data[user_name]["settings"].keys()!=def_settings.keys():
                for key in def_settings.keys():
                    if key not in self.users_data[user_name]["settings"]:
                        self.users_data[user_name]["settings"][key]=def_settings[key]
            if not "achievs" in self.users_data[user_name]:
                self.users_data[user_name]["achievs"]={}
                for ach in self.achievs_list:
                    self.users_data[user_name]["achievs"][ach]=False
    def get_user_data(self):
        self.check_user_data()
        self.best=self.users_data[self.user_name]["best"]
    ...
    def update_score(self):
        if self.user_name in self.users_data and self.game.is_debug==False:
            if (self.game.mode=="normal" and self.game.time>self.best[self.game.mode]["time"]) or (self.game.mode=="boss" and self.game.time<self.best[self.game.mode]["time"] and self.game.spawn_boss==True and len(self.game.boss)==0):
                self.best[self.game.mode]={
                    "time":self.game.time,
                    "seconds":self.game.seconds,
                    "minutes":self.game.minutes,
                    "version":version
                }
                self.users_data[self.user_name]["best"]=self.best
            self.save_data()
    def save_data(self):
        self.data["users"]=self.users_data
        with open("D:\\Crazy-Jump\\userdata.json","w",encoding="utf-8") as f:
    ...
    def login(self):
        ...
        self.inputbox.add(InputBox(50,190,250,50,"white","black","输入1则下次自动登录",tip_font=cn_sm,multiple_inputs=True))
                    ...
                    if bt.text=="登录" and bt.rect.y==250:
                        ...
                        if self.user_name in self.users_data:
                            if self.encrypt_password(input_psw)==self.users_data[self.user_name]["password"]:
                                ...
                                self.get_user_data()
                                self.save_data()
                                self.info=Info.append_info(self.info,"登录成功！",self.info_pos,color="green",time=1,back_color=None)
    ...
    def register(self):
        ...
        self.inputbox.add(InputBox(50,250,250,50,"white","black","输入1则下次自动登录",tip_font=cn_sm,multiple_inputs=True))
        ...
        self.get_user_data()
        self.save_data()
        self.info=Info.append_info(self.info,"注册成功！",self.info_pos,color="green",time=1,back_color=None)
    ...
    def logout(self):
        ...
        while running:
            ...
            clock.tick(60)
            pg.display.update()
        self.info=Info.append_info(self.info,"登出成功！",self.info_pos,color="green",time=1,back_color=None)
        while len(self.info)>0:
            self.game.event_handle()
            self.info.update()
            self.draw()
    ...
    def main(self):
        ...
        if self.best["normal"]["time"]==0:
            self.info=Info.append_info(self.info,f"普通模式最佳成绩：暂无",fst_pos,back_color=None,spacing=5,mode="back")
        elif self.best["normal"]["minutes"]>0:
            self.info=Info.append_info(self.info,f"普通模式最佳成绩：{self.best['normal']['minutes']}分{self.best['normal']['seconds']:.2f}秒",fst_pos,back_color=None,spacing=5,mode="back")
        else:
            self.info=Info.append_info(self.info,f"普通模式最佳成绩：{self.best['normal']['seconds']:.2f}秒",fst_pos,back_color=None,spacing=5,mode="back")
        if self.best["boss"]["time"]>=inf:
            self.info=Info.append_info(self.info,f"打boss模式最佳成绩：暂无",fst_pos,back_color=None,spacing=5,mode="back")
        elif self.best["boss"]["minutes"]>0:
            self.info=Info.append_info(self.info,f"打boss模式最佳成绩：{self.best['boss']['minutes']}分{self.best['boss']['seconds']:.2f}秒",fst_pos,back_color=None,spacing=5,mode="back")
        else:
            self.info=Info.append_info(self.info,f"打boss模式最佳成绩：{self.best['boss']['seconds']:.2f}秒",fst_pos,back_color=None,spacing=5,mode="back")
        running=True
        while running:
            events=self.game.event_handle()
            self.button.update(events)
    ...
    def change_password(self):
        ...
        self.save_data()
        self.info=Info.append_info(self.info,"密码修改成功！",self.info_pos,color="green",time=1,back_color=None)
        while len(self.info)>0:
            self.game.event_handle()
            self.info.update()
            self.draw()
    ...
    def danger_operation(self):
        ...
        while running:
            ...
            for bt in self.button:
                if bt.check_click():
                    ...
                    button2.empty()
                    button2.add(Button((310,130),250,50,"blue",font=cn_sm,text=f"确认操作({cmd})"))
            for bt in button2:
                if bt.check_click():
                    if bt.rect.y==130:
                        if input_psw=="":
        ...
        if cmd=="注销":
            ...
            self.users_data[self.user_name]["best"]={
                "normal":{
                    "time":0,
                    "seconds":0,
                    "minutes":0,
