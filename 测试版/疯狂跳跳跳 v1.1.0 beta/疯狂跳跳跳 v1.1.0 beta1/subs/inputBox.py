...
class InputBox(pg.sprite.Sprite):
    def __init__(self,x,y,width,height,outline_color,fill_color,tip,align="topleft",text="",is_selected=False,outline_width=2,max_len=20,tip_font=cn_def,input_font=cn_def,multiple_inputs=False,hidden_string=False):
        super().__init__()
        ...
        self.input_font_size=input_font.get_height()
        self.tip=tip
        self.input_text=text
        self.image=pg.Surface((self.w,self.h))
        self.image_rect=self.image.get_rect()
        self.tip_img=self.tip_font.render(tip,True,"gray")
        self.input_text_img=input_font.render(self.input_text,True,"white")
        for i in range(ord('a'),ord('z')+1):
            self.allow_keys.append(chr(i))
        for i in range(ord('A'),ord('Z')+1):
            self.allow_keys.append(chr(i))
        for i in range(ord('0'),ord('9')+1):
            self.allow_keys.append(chr(i))
        self.allow_keys.extend(['!','@','#','$','%','^','&','*','(',')','-','_','+','=','{','}','[',']','\\','|',';',':',"'",'"',',','.','/','<','>','?'])
        if align=="topright":
            x-=self.w
        elif align=="topleft":
            pass
            ...
            add_error(f"[error:InputBox/__init__]align({align})不存在")
        if text!="":
            if self.hidden_string:
                self.cursor_left_text_width=self.input_font.render("*"*len(text),True,"white").get_width()
            else:
                self.cursor_left_text_width=self.input_font.render(text,True,"white").get_width()
    ...
    def draw_picture(self):
        ...
        if self.input_text=="":
            ...
            if self.hidden_string:
                self.input_text_img=self.input_font.render("*"*len(self.input_text),True,"white")
            else:
                self.input_text_img=self.input_font.render(self.input_text,True,"white")
        ...
        if self.cursor_is_show:
            ...
            if self.hidden_string:
                input_text_left=self.input_font.render("*"*len(self.input_text),True,"white").get_rect(center=(self.w/2,self.h/2)).x
            cursor_x=input_text_left+self.cursor_left_text_width
            pg.draw.line(self.image,"white",(cursor_x,self.h/2-self.input_font_size/2),(cursor_x,self.h/2+self.input_font_size/2),2)
    def handle_key(self,key):
        ...
        if key==pg.K_RETURN:
            ...
            if self.input_text!="" and self.cursor_pos>0:
                img=self.input_font.render(self.input_text[self.cursor_pos-1],True,"white")
                if self.hidden_string:
                    img=self.input_font.render("*",True,"white")
            ...
            if self.cursor_pos<len(self.input_text):
                img=self.input_font.render(self.input_text[self.cursor_pos],True,"white")
                if self.hidden_string:
                    img=self.input_font.render("*",True,"white")
                self.cursor_left_text_width+=img.get_width()
                self.cursor_pos+=1
    ...
    def update(self,events):
        ...
        for ev in events:
            if ev.type==pg.MOUSEBUTTONDOWN:
                if self.rect.collidepoint(ev.pos):
                    ...
                    if self.hidden_string:
                        self.cursor_left_text_width=self.input_font.render("*"*len(self.input_text),True,"white").get_width()
                    else:
                        self.cursor_left_text_width=self.input_font.render(self.input_text,True,"white").get_width()
                else:
                    self.is_selected=False
                    self.cursor_is_show=False
                    self.cursor_change_time=0.5
        ...
        backup_cd_keys_time=copy.deepcopy(self.cd_keys_time)
        for key in backup_cd_keys_time:
            if keys[key]:
                ...
                self.cd_keys_time.pop(key)
        backup_pressing_keys=copy.deepcopy(self.pressing_keys)
            ...
            if keys[key]:
                if 32<=key<=126:
                    ...
                    if self.hidden_string:
                        key=ord("*")
                    self.cursor_pos+=1
                    img=self.input_font.render(chr(key),True,"white")
                    self.cursor_left_text_width+=img.get_width()
                else:
                    self.handle_key(key)
            else:
        ...
        for ev in events:
            if ev.type==pg.KEYDOWN:
                is_valid=False
                if ev.unicode and (ev.key not in self.other_keys) and ev.unicode in self.allow_keys:
                    self.input_text=self.input_text[:self.cursor_pos]+ev.unicode+self.input_text[self.cursor_pos:]
                    if self.hidden_string:
                        ev.unicode="*"
                    self.cursor_pos+=1
                    img=self.input_font.render(ev.unicode,True,"white")
                    self.cursor_left_text_width+=img.get_width()
                    is_valid=True
                elif ev.key in self.other_keys:
