...
class Enemy(pg.sprite.Sprite):
    ...
    def update(self):
        ...
        if rd.random()<=0.001:
            ...
            self.b-=8
            if self.speed>0:
                if self.speed-1<self.minspeed-1:
                    self.kill()
                self.speed=max(self.minspeed-1,self.speed-1)
            else:
                if self.speed+1>-self.minspeed+1:
                    self.kill()
                self.speed=min(-self.minspeed+1,self.speed+1)
